...
#read the .csv file containing all the SGD/CNY related information
sgdcny <- read.csv(file="SGD_CNY.csv", header = T)
dates <- as.Date(as.character(sgdcny$Date), format="%m/%d/%Y")
#log returns ... there are several values: open price / high price / low price / (closing) price 
returns <- diff(log(sgdcny$Price))
